...
def buttonAddClick():
    ...
    if name == '':
        tkinter.messagebox.showerror(title='出现错误', message='姓名不能为空')
        return
    ...
    if c!=0:
        tkinter.messagebox.showerror(title='出现错误', message='姓名不能重复')
        return
    
    # 获取选择的性别
    sex = comboSex.get()
    if sex not in ('男', '女'):
        tkinter.messagebox.showerror(title='出现错误', message='请输入正确的性别')
        return
    
    # 检查年龄
    age = entryAge.get().strip()
    if not age.isdigit():
        tkinter.messagebox.showerror(title='出现错误', message='年龄必须为数字')
        return
    if not 1<int(age)<100:
        tkinter.messagebox.showerror(title='出现错误', message='年龄必须在1到100之间')
        return

    # 检查部门
    department = entryDepartment.get().strip()
    if department == '':
        tkinter.messagebox.showerror(title='出现错误', message='部门不能为空')
        return
    
    # 检查电话号码
    telephone = entryTelephone.get().strip()
    if telephone=='' or (not telephone.isdigit()):
        tkinter.messagebox.showerror(title='出现错误', message='电话号码必须为数字')
        return
    
    # 检查QQ号码
    qq = entryQQ.get().strip()
    if qq=='' or (not qq.isdigit()):
        tkinter.messagebox.showerror(title='出现错误', message='QQ号码必须为数字')
        return
...
def buttonDeleteClick():
    ...
    if name == '':
        tkinter.messagebox.showerror(title='出现错误', message='请选择一条记录')
        return
