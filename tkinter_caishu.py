...
root.attributes("-topmost", True)     # 永远处于顶层
...
labelDataTime = tkinter.Label(root,width = 130)
labelDataTime.pack(fill = tkinter.BOTH,expand = tkinter.YES)
labelDataTime.configure(bg = 'yellow')
...
labelDataTime.bind('<Button-1>',onLeftButtonDown)

def onLeftButtonUp(event):
    root.attributes('-alpha',0.9)
    canMove.set(0)
    
labelDataTime.bind('<ButtonRelease-1>',onLeftButtonDown)

def quit_button(event):
    but1 = tkinter.Button(root, text = '退出', width = 10,height = 2,command = root.quit)
    but1.pack()
