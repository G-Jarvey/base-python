...
#放置清除按钮和等号按钮
btnClear = tkinter.Button(root,text='Clear',bg = 'red',command = lambda:buttonClick('C'))  #在root主界面中放置按钮，按钮上显示Clear，红色，点击按钮后进入buttonClik回调函数做进一步的处理，注意传入了参数‘C’，这样就能分清是哪个按钮按下了
#下面的内容和上面的模式都是一样的
btnClear.place(x = 40,y = 40,width = 80,height = 20)
btnCompute = tkinter.Button(root,text = '=',bg = 'yellow',command = lambda :buttonClick('='))
...
for row in range(4):
    for col in range(3):
        ...
        index += 1  #索引号递增
        btnDigit = tkinter.Button(root,text = d,bg = 'gray',command = f)#和上面的是类似的
...
for index,operator in enumerate(operators):
    ...
    btnOperator = tkinter.Button(root,text = operator,bg = 'orange',command = f)#创建的过程和上面类似
